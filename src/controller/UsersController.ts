...
export const activatedUser = async (request: Request, response: Response) => {
	const { id } = request.params
	
	const user = await getRepository(Users).update(id, {
	 	activated: true
	})
	...
	if (user.affected === 1) {
 	...
 	return response.status(404).json({ message: 'User not active yet'})
};
